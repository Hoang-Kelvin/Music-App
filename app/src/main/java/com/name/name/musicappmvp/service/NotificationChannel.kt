...
class NotificationChannel {
    ...
    fun songNotification(context: Context, tilted: String) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(ChannelEntity.NOTIFY_ID.toString(),
                context.getString(R.string.notify_playing),NotificationManager.IMPORTANCE_DEFAULT)
            val serviceNotification = context.getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager
        ...
        notification1 = notificationBuilder.setOngoing(true)
            ...
            .setContentTitle(context.getString(R.string.notify_playing))
